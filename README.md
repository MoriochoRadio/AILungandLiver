...
## 🧬 L-POT 이라는 이름에 관하여 (정직하게)

제품명 **L-POT** 의 약자 의미는, 솔직히 말하면 본인도 정확히 모릅니다. 추정하기로는:

- **L** = **L**ung + **L**iver (폐·간 두 부위, 둘 다 L 로 시작)
- **P** = 결절을 뜻하는 s**P**ot 의 P
- **O** = 탐지 (detect)
- **T** = 분석 (analyze)

정도로 짐작하지만, 발표 PPT 15 슬라이드와 162 페이지 산출물 어디에도 약자 풀이가 명시되어 있지 않습니다. 학기 막바지에 팀 안에서 자연스럽게 굳어진 이름인 것 같습니다. 그럴듯하게 지어내기보다 *"정확히는 모른다"* 고 적어두는 게 맞다고 생각합니다.
